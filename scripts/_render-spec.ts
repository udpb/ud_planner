/**
 * DECK-3 (ADR-025 Phase 3) — 스펙↔렌더 계약 검증 하니스 (결정론적, LLM·DB 없음)
 *
 * 실행: npx tsx scripts/_render-spec.ts
 *
 * docs/samples/fixtures/deckspec-B2G.json (손작성 DeckSpec) 을
 *   load → zod 검증(parseDeckSpec, deckSpecToElements 내부) → deckSpecToElements →
 *   renderToStaticMarkup → 자체완결 HTML → playwright chromium →
 *     - docs/samples/sample-deckspec-v3.pdf      (고해상 PDF)
 *     - docs/samples/sample-deckspec-v3-p1.png   (1페이지 스냅샷)
 *     - docs/samples/snaps-spec/p{n}.png         (전 페이지 PNG — 육안 검증)
 * + 슬라이드별 밀도 측정표 (정보 블록 수 · 근거 밴드 · dead-space).
 *
 * 합격선 (DECK-3 브리프 §5 — DECK-2 동등 밀도가 JSON 으로 구동됨을 증명):
 *   - 본문 슬라이드(표지/디바이더/마무리 제외) 평균 정보 블록 ≥ 11
 *   - 본문 슬라이드 평균 dead-space < 15%
 *   - 모든 본문 슬라이드에 근거 밴드 존재
 *   - 유효 PDF · PDF 페이지 = 슬라이드 수 · 16:9 · 한글 · 폰트 임베드
 *   - 리치 어휘 ≥ 3종 (아이콘/이미지/로고)
 *
 * ⚠️ 긴 백그라운드 프로세스 금지 — 1회 렌더 후 종료. LLM/DB 호출 없음.
 */
...
const OUT_PDF = path.join(process.cwd(), 'docs', 'samples', 'sample-deckspec-v3.pdf')
const OUT_PNG = path.join(process.cwd(), 'docs', 'samples', 'sample-deckspec-v3-p1.png')
const SNAP_DIR = path.join(process.cwd(), 'docs', 'samples', 'snaps-spec')
...
async function main() {
  // 1. fixture JSON 로드 + zod 검증 (deckSpecToElements 내부 parseDeckSpec)
  const raw = JSON.parse(fs.readFileSync(FIXTURE, 'utf-8'))
  const slides = deckSpecToElements(raw)
  console.log(`\nfixture loaded + zod-validated: ${path.basename(FIXTURE)}`)
  console.log(`slides rendered from spec: ${slides.length}`)
  ...
  console.log('')
  if (fails.length === 0) {
    console.log('✅ PASS — JSON DeckSpec 이 DECK-2 동등 밀도로 렌더됨 (스펙↔렌더 계약 성립).')
  } else {
    console.log('❌ FAIL:')

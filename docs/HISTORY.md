...
| 문서 | 상태 |
|---|---|
| **`docs/UD-Engine-PRD-v2.0.html`** ⭐⭐ | **단일 진실 PRD (2026-06-04)** — 출력 구조 전환(덱-우선 HTML 렌더) 반영. 제품 프로세스·유저플로우 **SVG 도식** + 실제 자동생성 슬라이드 **목업 임베드**(prd-v2-assets/) + 유지/고도화/폐기/추가 + 로드맵. 디자인 킷. 렌더 검증 완료. |
| `docs/UD-Engine-PRD-v1.0.html` | ⚠️ **v2.0 으로 승계** (2026-06-04) — 생성 엔진 중심(deck-first 전환 전). 이력 참조용. |
| **`docs/UD-Engine-JourneyMap-v1.0.html`** | PM 저니맵 (목업, 2026-06-01) — S0~S7·9결정지점. ⚠️ 덱-우선 전환분은 PRD-v2.0 유저플로우가 갱신. |

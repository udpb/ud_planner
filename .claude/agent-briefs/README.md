...
## 활성 브리프
- `DECK-2-density-detail-proof.md` — 🟡 in-progress (2026-06-01). **ADR-025 Phase 2.** 본문 슬라이드를 당선 덱 밀도(블록~12)·요소별 디테일(코치 약력+실적·주차별 커리큘럼·정량 근거)로 손작성 proof + 리치 컴포넌트 라이브러리 확정. DECK-3(자동저작)이 목표 삼을 밀도 규격. 검증=결정론적 PDF+전페이지 PNG.

...
### 재정렬 방향 (다음 세션 첫 일)
- **최초 기획부터 현 구현까지 전수 재점검 → 전체 align.** 메모리 [[reference-udplanner-feature-map]] + 아래 "여정" 참조.
- **화면 세부 목업 전부 + 저니맵 + 디테일 구조** 작성(사용자 명시).
- 5개 피드백을 설계 요구사항으로: 대화형 브레인 + PM 편집 + 진행/목표 가시화 + 임팩트 동작.
- ⚠️ **또 성급히 빌드 금지** — 메인이 제품을 반복 과소파악했음(코치풀·자동지능 누락 전력). 충분히 align 먼저.

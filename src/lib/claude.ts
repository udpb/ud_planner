...
import { invokeAi } from './ai-fallback'
...
// ────────────────────────────────────────────────────────────────
// JSON 파서 — 2026-05-03 Phase 2: src/lib/ai/parser.ts 로 분리
// 본 파일에서는 호환성을 위해 re-export.
// ────────────────────────────────────────────────────────────────
export {
  JsonParseError,
  safeParseJson as safeParseJsonExternal,
} from './ai/parser'
import { safeParseJson } from './ai/parser'

// ────────────────────────────────────────────────────────────────
// 전략 맥락 — 2026-05-03 Phase 2: src/lib/ai/strategic-notes.ts 로 분리
// ────────────────────────────────────────────────────────────────
export {
  formatStrategicNotes,
  type StrategicNotes,
} from './ai/strategic-notes'
import { formatStrategicNotes, type StrategicNotes } from './ai/strategic-notes'

// ────────────────────────────────────────────────────────────────
// 외부 리서치 — 2026-05-03 Phase 2: src/lib/ai/research.ts 로 분리
// ────────────────────────────────────────────────────────────────
export {
...
import { formatExternalResearch, type ExternalResearch } from './ai/research'
...
export async function parseRfp(text: string): Promise<RfpParsed> {
  // 2026-05-03: anthropic 직접 호출 → invokeAi 마이그 (Gemini Primary + Claude Fallback)
...
export async function suggestImpactGoal(
  ...
  // 2026-05-03: anthropic → invokeAi 마이그
  const result = await invokeAi({

...
- 운영 유형 ${operatingType} 의 성격에 맞게 (T1 정규강좌=매주 / T2 몰입캠프=몰아치기 / T3 장기여정=킥오프+퀘스트+발표 조합).
  ...
  "sessions": [
    {
      ...
      "rationale": "이 회차가 왜 여기에 있는지 (흐름 문법 근거)"
    }
  ]
}`
...
async function generateNonSessionStructure(
  ...
  const shape = isEvent
    ? `T5 행사 운영형 — 본체가 커리큘럼이 아니라 **행사 설계**입니다 (경진대회·박람회·공모전 운영 대행).
**절대 주차별 회차표를 만들지 마세요.** 행사 준비·운영·사후의 단계(stage)로 설계하세요. (v1.2 §09-C)`
    : `T4 개별 밀착형 — 팀이 아니라 **개별 사업체 단위**입니다 (소상공인·재창업).
**절대 주차별 회차표를 만들지 마세요** — 점포/사업체마다 일정이 제각각이라 정기 모임은 인풋 낭비입니다.
진단 방문 → 공통 접점(최소) → 개별 컨설팅 → AI코치 상시 의 단계(stage)로 설계하세요. (v1.2 §09-B)`
...
아래 JSON 형식만 반환 (마크다운 코드블록 없이):
{
  "kind": "${kind}",
  "stages": [
    { "label": "단계 라벨 (예: 진단 방문)", "content": "이 단계에서 하는 일", "rationale": "근거 (v1.2 또는 확정 결정)" }
  ]
}`

...
- goalInterpretation(목표 해석): RFP 목표를 우리 관점으로 재해석. RFP 에 근거가 충분하면 "high".
- yearOverYear(작년 대비): 작년 운영 대비 무엇이 달라져야 하나. **작년 운영·담당자 암묵지는 당신이 알 수 없으므로**, 일반적 가설만 제시하고 confidence="low" 로 두세요(PM 이 대화로 채움). 단서가 RFP 에 명시돼 있으면만 "high".
- differentiation(차별점): 우리(언더독스)의 우위·차별점. 매칭 자산에 근거가 있으면 "high".
- risk(리스크): 담당자가 우려할 핵심 리스크. **담당자 우려는 사람만 아는 영역**이므로 일반적 리스크 가설만 제시하고 confidence="low" 로 두세요. RFP 의 제약·평가배점에서 명백히 드러나면만 "high".

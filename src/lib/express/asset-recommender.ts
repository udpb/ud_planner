...
export async function recommendAssetsForWeakLenses(
  ...
  const { weakLenses, programProfile, topNPerLens = 3, totalLimit = 6 } = params
  ...
  // 한 번에 모든 stable 자산 로드 (자산 수 < 1000 가정 — Wave N4 에서 vector 로 대체)
  ...
  for (const lens of targetLenses) {
    ...
    const scored = dbAssets
      .map((asset) => {
        ...
        // 기본 점수: evidence (60%) + category (40%)
        const base = 0.6 * evScore + 0.4 * catScore
        ...
        const totalScore = Math.min(1, base + profileBonus)

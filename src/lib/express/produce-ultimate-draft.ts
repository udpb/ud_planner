...
export async function produceUltimateDraft(
  ...
  if (emptySections.length > 0) {
    ...
    const t32 = Date.now()
    // 15H-Phase2(concreteness): 섹션 유형별 '실행 실체' 스캐폴드 — 평가위원 6/6 최약점(avg 61).
    //   기존엔 규칙 한 줄로 묻혀 LLM 이 약하게 취급 → 섹션별 구조화 요구로 격상.
    const CONCRETE: Record<string, string> = {
      '1': '구체성: 이 사업이 *지금* 필요한 정책·시장 근거에 연도·수치 최소 1개. 추상적 당위 금지.',
      '2': '구체성: 전략을 단계(STEP/Phase)로 나누고 각 단계 착수 시기(개월/주차 범위)+도달 산출물 명시.',
      '3': '구체성(필수): 진행 흐름을 "주차/단계 — 핵심 활동 — 산출물" 4~6행으로. RFP 기간·정원·대상단계 반영, 대면/온라인·거점(지역 기반) 명시. ※교육형이 아니면(축제·행사·플랫폼) 회차 대신 그 사업의 실행 단위(사전기획→운영→사후 등)로.',
      '4': '구체성(필수): PMO 역할 분담(전담 PM·운영·코치 R&R) + 발주처 보고 주기(주간/월간) + 거점·장소 + 중도이탈(retention) 대응 1개 구체 + 결과보고서 체계.',
      '6': '구체성: KPI 3~4개 각각 "정량 목표 + 측정 방법 + 측정 시점". "80% 달성"만 표기 금지.',
    }
    const ANTI_FAB =
      '※ 구체성 ≠ 날조: RFP 의 기간·정원·지역·산출물에서 도출한 일정·거점·산출물을 제시하되, 미확인 협력기관 실명·없는 수치는 만들지 말고 유형(예: "광역 창조경제혁신센터·지역 대학 LINC사업단")으로 표현.'
...
3. 발주처 키워드 자연스럽게 흡수
4. 추상적 나열 X — **${CONCRETE[sec.n] ?? '단계·항목·정량 구체화 (평가위원은 실행 실체를 본다)'}**
   ${ANTI_FAB}

 ...
 * 자산만 모은다.
 *
 * 동시성: 섹션·과업 검색은 retrieve() 내부가 이미 병렬이므로 여기서는 순차로 호출해
 * 외부 LLM(rerank 등) 동시성 폭주(429)를 피한다. gather 는 본문 작성(assemble)보다
 * 가벼우므로 순차로도 충분.
 */

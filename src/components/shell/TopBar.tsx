...
export function TopBar({
  ...
  return (
    <header
      className="sticky top-0 z-30 flex h-14 items-center gap-6 px-8 text-white"
      style={{
    ...
    >
      ...
      <Link
        href="/"
        className="text-[17px] font-bold italic tracking-tight text-white"
      >
        UD·Planner
      </Link>
      ...
      {/* v2 badge */}
      <span
        className="px-2 py-0.5 text-[9px] font-semibold uppercase tracking-[1.5px]"
        style={{
          color: 'var(--action-orange)',
          border: '1px solid rgba(255,130,4,.45)',
        }}
      >
        v2
      </span>
      ...
      {projectSwitchHref ? (
        <Link
          ...
          className="flex max-w-[280px] items-center gap-1 truncate px-3 py-1.5 text-[13px] font-medium text-white/85 transition-colors hover:text-[var(--action-orange)]"
        >
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="inline-flex h-8 items-center gap-1.5 px-3.5 text-[11px] font-semibold uppercase tracking-[0.5px] transition-colors"
          style={{
...
function StageStep({
  ...
  return (
    ...
    >
      ...
      {!isLast && (
        <span
          className="absolute right-[-8px] top-1/2 -translate-y-1/2 text-[14px]"
